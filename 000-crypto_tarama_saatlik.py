# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 13:33:29 2024
...
def Tarama_1():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    ...
                    Column('RSI|60').between(30,40),
                    Column('change|60').between(0,5.0),
                    Column('MACD.macd|60').crosses_above(Column('MACD.signal|60')),
                    Column('MACD.signal|60') < 0
                    )
        .get_scanner_data())[1]
...
def Tarama_2():
    ...
    Taramasıdır.
    #Alternatif Düşeni kıran taraması olarak bilinir.
    """
    Tarama = (Query().set_markets('crypto')
                .select('name', 'change','close','volume','Perf.W')
                .where(
                    Column('exchange')=='BINANCE',
                    Column('Perf.W') < 15,
                    Column('relative_volume_10d_calc|60') > 1.5,
                    Column('change|60') < 5.0,
                    Column('EMA5|60') < Column('close|60'),
                    Column('EMA20|60') < Column('EMA5|60'),
                    Column('EMA50|60') < Column('EMA20|60'),
                    Column('MACD.macd|60') > Column('MACD.signal|60'),
                    Column('P.SAR|60').crosses_below(Column('close|60')),
                    Column('CCI20|60') >= 90.0
                    )
        .get_scanner_data())[1]
...
def Tarama_3():
    ...
    Taramasıdır.
    #Swint Trade 2 nolu stratejisine uyan tarama olarakda bilinir.
    """
    Tarama = (Query().set_markets('crypto')
                .select('name', 'change','close','volume','Perf.W')
                .where(
                    ...
                    Column('Perf.W') < 15.0,
                    Column('change|60') < 5.0,
                    Column('relative_volume_10d_calc|60')>1.0,
                    Column('close|60') >= Column('SMA5|60'),
                    Column('SMA10|60') >Column('SMA20|60'),
                    Column('MACD.macd|60').crosses_above(Column('MACD.signal|60'))
                    )
        .get_scanner_data())[1]
...
def Tarama_4():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    ...
                    Column('relative_volume_10d_calc|60')>1.0,
                    Column('MACD.macd|60').crosses_above(Column('MACD.signal|60')),
                    Column('Stoch.RSI.K|60').crosses_above(Column('Stoch.RSI.D|60'))
                    )
        .get_scanner_data())[1]
...
def Tarama_5():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    Column('exchange')=='BINANCE',
                    Column('close|60').crosses_above('HullMA9|60'),
                    Column('change|60') < 5.0,
                    Column('ATR|60').between(0,10),
                    Column('SMA20|60').crosses_below('close|60'),
                    )
        .get_scanner_data())[1]
...
def Tarama_6():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    ...
                    Column('change|60') < 5.0,
                    Column('EMA5|60') < Column('close|60'),
                    Column('EMA20|60') < Column('SMA5|60'),
                    Column('EMA50|60') < Column('SMA20|60'),
                    Column('CCI20|60').crosses_above(100),
                    Column('Value.Traded|60') > 2.5E6
                    )
        .get_scanner_data())[1]
...
def Tarama_8():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    ...
                    Column('Perf.W') < 10.0,
                    Column('change|60') < 5.0,
                    Column('relative_volume_10d_calc|60')>1.3,
                    Column('MACD.macd|60').crosses_above(Column('MACD.signal|60')),
                    Column('Stoch.RSI.K|60').crosses_above(Column('Stoch.RSI.D|60'))
                    )
        .get_scanner_data())[1]
...
def Tarama_9():
    ...
    Tarama = (Query().set_markets('crypto')
                ...
                .where(
                    ...
                    Column('change|60') < 5.0,
                    Column('relative_volume_10d_calc|60')>1.5,
                    Column('MACD.macd|60').crosses_above(0),
                    Column('MACD.macd|60') > (Column('MACD.signal|60')),
                    )
        .get_scanner_data())[1]
