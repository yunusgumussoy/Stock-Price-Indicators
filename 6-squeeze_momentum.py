...
def SqueezeMomentum(data,mult=2,length=20,multKC=1.5,lengthKC=20):
    df=data.copy()
    df['basis']=ta.sma(data['Close'],length) # Bollinger Bands
    df['dev']=multKC*ta.stdev(data['Close'],length)
    df['upperBB']=df['basis']+df['dev']
    df['lowerBB']=df['basis']-df['dev']
    df['ma']=ta.sma(df['Close'],lengthKC)   # Keltner Channels
    ...
    df['Squeeze'] = (df['lowerBB'] < df['lowerKC']) & (df['upperBB'] > df['upperKC']) # A squeeze occurs when the Bollinger Bands are within the Keltner Channels.
    return df
