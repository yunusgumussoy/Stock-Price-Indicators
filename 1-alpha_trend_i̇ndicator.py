...
from tradingview_screener import get_all_symbols # to get symbols from TradingView
import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)

def Alpha_Trend(df, mult=1, n1=14):
    df['TR'] = ta.true_range(df['High'],df['Low'],df['Close']) # True Range (TR)
    df['ATR'] = ta.sma(df['TR'], length=n1) # Average True Range (ATR)
    df['mfi'] = ta.mfi(df['High'],df['Low'],df['Close'], df['Volume'], length=n1) # Money Flow Index (MFI)
...
# Connects to TradingView to fetch symbols for Turkish stocks
tv = TvDatafeed()
